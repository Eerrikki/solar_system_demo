...
@onready var _deep_space_player : AudioStreamPlayer = $DeepSpace
@onready var _planet_day_player : AudioStreamPlayer = $PlanetDay
@onready var _planet_night_player : AudioStreamPlayer = $PlanetNight
...
func _process(delta: float):
	...
	if planet.type != StellarBody.TYPE_MAIN_SEQUENCE_STAR:
		...
		planet_factor = clampf(planet_factor, 0.0, 1.0)
		...
		if planet.night_ambient_sound != null:
			...
			day_factor = clampf(sun_dp * 4.0 + 0.5, 0.0, 1.0)
